...
class BalanceTransferBankConfirmation extends Component {
  ...
  renderDateAndCamera () {
    const { dateTransfer, photo, calendarMonthYear, hour, minute } = this.state
    let time, color, tempHour, tempMinute
    if (!dateTransfer.includes('Pilih')) {
      if (hour !== '') {
        ...
        time = moment(dateTransfer).format('DD MMM YYYY').toString() + ' - ' + tempHour + ':' + tempMinute
      } else {
        time = moment(dateTransfer).format('DD MMM YYYY').toString()
      }
    ...
    return (
      <View style={[styles.inputContainer, { paddingBottom: ratioHeight(10) }]}>
        ...
        <CalendarModal
          ...
          showHour
          changeHour={(hour, minute) => this.changeHour(hour, minute)}
  ...
  render () {
    const { methodArray, showDropdown } = this.state
    return (
